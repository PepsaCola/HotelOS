...
interface ApprovalPagerProps {
  shown: number;
  total: number;
}

/** Presentational pager — wire to a paginated data source when the API lands. */
export function ApprovalPager({ shown, total }: ApprovalPagerProps) {
  return (
    <div className="flex flex-col items-start justify-between gap-3 border-t border-hair-2 px-3 py-3 sm:flex-row sm:items-center">
      <div className="text-[13px] text-muted-strong">
        Showing <b className="font-semibold text-ink-900">{shown}</b> of {total} pending items
      </div>
